...
from api_gateway_load.sensedia import ignore
...
class DataPrepare:
    ...
    def __init__(self, beginDate, endDate):
        ...
        self.collection_call_detail = self.mydb["sensedia-api-call-details"]
        self.collection_call_cleaned = self.mydb["sensedia-api-call-cleaned"]
        x = self.getApiCallsDetails(beginDate, endDate)   
        y = self.removeNoise(x)


    def getApiCallsDetails(self, beginDate, endDate):
    ...
    def removeNoise(self, api_calls):
        ...
        try:
            for call in api_calls:
                ...
                if "_source" in call and "http.status_code" in call["_source"]:
                    status = int(call["_source"]["http.status_code"])
                    if status >= 300:
                        continue
