...
def getLogs() -> List:
    ...
    try:
        ...
        ret = collection_calls.find()
        #df_aux = pandas.read_sql(ret)

        document_list = pandas.DataFrame(ret)
        ...
        for i, doc in enumerate(cursor_of_docs):
            ...
            document2 = pd.concat([document, serial_obj], ignore_index=True, axis="columns") # appending the serial_obj to the document series
            # clean null values
            document.dropna
...
def attributes_extraction(dataframe_json, doc_list_dataframe):
    ...
    try:
        if isinstance(dataframe_json, pandas.DataFrame):
            ...
            uri_obj_rec = None
            uri_obj_aux = domain.Uri.Uri(None, uri, client_id, method, None, request_timestamp)
            if not uri_obj_aux.is_valid():
                return None
            uri_obj_rec = MySqlRepository.find_uri(uri_obj_aux)
            if uri_obj_rec is None:
               #uri_obj_rec = MySqlRepository.MySqlRepository.create_uri(uri_obj_aux)
               uri_obj_rec = MySqlRepository.create_uri(uri_obj_aux)
            ...
            #trace_outer = df_doc.get("trace").to_string(index=False)
            ...
            for row in df_doc.itertuples(index=False, name=None):
                for key, value in enumerate(row):
                    ...
                    if ignore.field_to_ignore(outer_aux_attr_name) or value is None or str(value).isspace():
                        continue
                    ...
                    for aux_row in df_aux.itertuples():
                        ...
                        for aux_cols_i, aux_col_val in enumerate(aux_row):
                            ...
                            if aux_col_val == value:
                                uri_inner = domain.Uri.Uri(None, aux_row.__getattribute__("uri"), aux_row.__getattribute__("clientId"), aux_row.__getattribute__("receivedOnDate"))
                                ...
                                correlation = MySqlRepository.save_correlation(uri_obj_rec, uri_inner)
                                save_correlation = False  # The correlation was saved and do not need repeat or update it
                                if correlation is not None and isinstance(correlation, domain.Correlation.Correlation):
                                    ...
                                    if (len(trace_outer) > 0) and (len(trace_inner) > 0):
                                        ...
                                        #for aux_match_row in aux_match_values:
                                        #    print(aux_match_row)
                                        #save_matched_field_in_traces(trace_outer, trace_inner)
...
def save_correlation_matched_values(cor: domain.Correlation.Correlation, condition_attribute_name, result_attribute_name, attribute_value):
    # salva na correlaçao em repeated atributes o nome do atributo e o valor correspondente.
    try:
        if isinstance(cor, domain.Correlation.Correlation):  #Tinha hora que invés de objeto esta sendo passsado int
            rep_att = MySqlRepository.save_correlation_repeated_attributes(cor.get_id(), condition_attribute_name, result_attribute_name, attribute_value)
            print("save_correlation_matched_values:", rep_att.correlation_id)
    except Exception as error:
        print('Ocorreu problema {} '.format(error.__class__))
        print("mensagem", str(error))
        print("In save_correlation_matched_values module :", __name__)

# compare two traces of Sensedia API Gatewa. If their fields values match, it save the equality in correlation
def save_matched_field_in_traces(correlation: domain.Correlation.Correlation, trace1, trace2):
        ...
        print("save_matched_field_in_traces = ", flated_outer_body, " : ", flated_inner_body)
        ...
        if (correlation is not None) and (len(flated_outer_body) > 0) and (len(flated_inner_body) > 0):
            ...
            print(" Em save_matched_field_in_traces, um dos parametros está vazio")
